...
- 新規運用では `profile` / `プロフィール` フォルダへプロフィール画像を1枚だけ置く。大きく表示する `imageUrl` と、プロフィール帳の**上中央にある写真枠の中心部を1.25倍ズームで切り出して256pxへ拡大する**一覧用 `iconUrl` をCloudinaryの変換URLで自動作成・登録する。元画像やアイコン用の複製ファイルは増えない。共通テンプレートの2000px幅を基準にするため、縮小版の同じレイアウトにも対応する。
...
`mentorId` は変更しないでください。既存のQR紐付けと獲得履歴を残したまま、画像だけを更新できます。Cloudinaryはプロフィール帳の**上中央にある写真枠**を256×256の正方形アイコンとして配信します。画像ファイルを置かずに反映すると、既存画像URLを維持します。
