...
- 新規運用では `profile` / `プロフィール` フォルダへプロフィール画像を1枚だけ置く。大きく表示する `imageUrl` と、プロフィール帳の**上中央にある写真枠だけを切り出して256pxへ拡大する**一覧用 `iconUrl` をCloudinaryの変換URLで自動作成・登録する。元画像やアイコン用の複製ファイルは増えない。共通テンプレートの2000px幅を基準にするため、縮小版の同じレイアウトにも対応する。
